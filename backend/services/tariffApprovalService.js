...
const submit=async({reference,auth,executor=db})=>{
 ...
 await writeAuditLog({user_id:auth?.userId||null,action:"SUBMIT_TARIFF_APPROVAL",module:"Tariff Governance",description:`Submitted tariff ${reference} for Management approval.`,metadata:{entity_reference:reference,rate:row.daily_rate,currency:row.currency,late_collection_penalty_percent:row.late_collection_penalty_percent}},executor);
...
const decide=async({reference,decision,reason,auth,executor=db})=>{
 ...
 await executor.query(`UPDATE tariff_versions SET approval_status=$1::text,approved_by=CASE WHEN $1::text='APPROVED' THEN $2::integer END,approved_at=CASE WHEN $1::text='APPROVED' THEN CURRENT_TIMESTAMP END,rejected_by=CASE WHEN $1::text='REJECTED' THEN $2::integer END,rejected_at=CASE WHEN $1::text='REJECTED' THEN CURRENT_TIMESTAMP END,rejection_reason=CASE WHEN $1::text='REJECTED' THEN $3::text END,is_active=CASE WHEN $1::text='REJECTED' THEN FALSE ELSE is_active END,updated_at=CURRENT_TIMESTAMP WHERE id=$4`,[decision,auth?.userId||null,String(reason||"").trim()||null,row.id]);
 ...
 await writeAuditLog({user_id:auth?.userId||null,action:`${decision}_TARIFF`,module:"Tariff Governance",description:`${decision} tariff ${reference}.`,metadata:{entity_reference:reference,reason:reason||null,submitted_by:row.submitted_by,late_collection_penalty_percent:row.late_collection_penalty_percent}},executor);
