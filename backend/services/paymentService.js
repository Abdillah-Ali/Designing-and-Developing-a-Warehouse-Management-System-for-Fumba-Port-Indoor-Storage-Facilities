...
const ensureAutomaticInvoice = async ({ cargoReference, executor = db }) => {
  ...
  const updated = await executor.query(`UPDATE invoices SET auto_generated=TRUE,payment_reference=$1,payment_public_token=COALESCE(payment_public_token,$2),issued_by=NULL,updated_at=CURRENT_TIMESTAMP WHERE public_invoice_number=$3 RETURNING *`, [paymentReference, publicToken, draft.invoice_number]);
