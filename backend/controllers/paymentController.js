...
const { initiatePayment,processWebhook,ensureAutomaticInvoice }=require("../services/paymentService");
...
const webhook=async(req,res,next)=>{try{const data=await tx(c=>processWebhook({headers:req.headers,payload:req.body,executor:c}));res.json({success:true,data})}catch(e){next(e)}};
