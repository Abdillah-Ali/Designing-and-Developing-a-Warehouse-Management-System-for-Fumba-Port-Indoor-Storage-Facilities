...
import { AlertTriangle, BarChart3, Bell, ClipboardCheck, Database, LayoutDashboard, LogOut, UserCircle2, X } from "lucide-react";
...
function Dashboard() {
  ...
  const alerts = [
    ["Customs Backlog", metrics.customs_backlog, "Cargo awaiting Customs action."],
    ["Bins At Capacity", metrics.bins_at_capacity, "Bins that reached a configured physical limit."],
    ["Outstanding Invoices", metrics.outstanding_invoices, "Non-cancelled invoices with a balance."],
    ["Release Blockers", metrics.blocked_releases, "Cargo blocked by release-readiness rules."]
  ];
  return <>
    ...
    <div className="flex-1 overflow-auto p-4">
      ...
      <SectionCard title="Alerts" icon={AlertTriangle} className="mt-4">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {alerts.map(([title, value, description]) => <div key={title} className="rounded border-l-4 border-amber-500 bg-amber-50 p-3"><div className="flex justify-between gap-3 text-sm font-semibold"><span>{title}</span><span>{state.loading ? "…" : value ?? 0}</span></div><p className="mt-1 text-xs text-muted-foreground">{description}</p></div>)}
        </div>
      </SectionCard>
    </div>
  </>;
